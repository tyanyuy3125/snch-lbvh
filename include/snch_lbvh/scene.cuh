...
namespace lbvh
{
    ...
    public:
        struct silhouette_vertex
        {
            ...
            __device__ __host__ float2 normal(int f_index, bool normalize = true) const;
    ...
    public:
        struct silhouette_edge
        {
            ...
            __device__ __host__ bool has_face(int f_index) const;
            __device__ __host__ float3 normal(int f_index, bool normalize = true) const;
    ...
    __device__ __host__
    scene<2>::line_segment::line_segment(const int2 vertex_indices, const int2 silhouette_indices, const thrust::device_vector<float2> &vertices_d, const thrust::device_vector<silhouette_vertex> &silhouettes_d)
    ...
    __device__ __host__ lbvh::aabb<float, 2> scene<2>::aabb_getter::operator()(const line_segment &ls) const noexcept
    {
    ...
    __device__ __host__ lbvh::cone<float, 2> scene<2>::cone_getter::operator()(const line_segment &ls) const noexcept
    {
    ...
    __device__ __host__ float scene<2>::distance_calculator::operator()(const float2 point, const line_segment &object) const noexcept
    {
    ...
    __device__ __host__ bool scene<2>::silhouette_distance_calculator::operator()(const float2 origin, const line_segment &object, const float max_radius_squared, float &distance,
                                                                                  const bool flip_normal_orientation, const float min_radius_squared) const noexcept
    {
    ...
    template <typename VerticesInputIterator, typename IndicesInputIterator>
    scene<2>::scene(VerticesInputIterator vertices_first, VerticesInputIterator vertices_last,
                    IndicesInputIterator indices_first, IndicesInputIterator indices_last)
        : vertices_h(vertices_first, vertices_last), vertices_d(vertices_h), indices_h(indices_first, indices_last)
    {
    }

    __device__ __host__ scene<2>::silhouette_vertex::silhouette_vertex(const int4 indices, const thrust::device_vector<float2> &vertices_d)
        : indices(indices), vertices(vertices_d.data().get())
    {
    }

    __device__ __host__ scene<2>::silhouette_vertex::silhouette_vertex(const thrust::device_vector<float2> &vertices_d)
        : indices(make_int4(-1, -1, -1, -1)), vertices(vertices_d.data().get())
    {
    }

    __device__ __host__ scene<2>::silhouette_vertex::silhouette_vertex()
        : indices(make_int4(-1, -1, -1, -1)), vertices(nullptr)
    {
    }

    __device__ __host__ aabb<float, 2> scene<2>::silhouette_vertex::bounding_box() const
    {
    ...
    __device__ __host__ float2 scene<2>::silhouette_vertex::centroid() const
    {
        const float2 &p = vertices[get(indices, 1)];
        return p;
    }

    __device__ __host__ bool scene<2>::silhouette_vertex::has_face(int f_index) const
    {
        return f_index == 0 ? get(indices, 2) != -1 : get(indices, 0) != -1;
    }

    __device__ __host__ float2 scene<2>::silhouette_vertex::normal(int f_index, bool do_normalize) const
    {
        int i = f_index == 0 ? 1 : 0;
    ...
    __device__ __host__ float2 scene<2>::silhouette_vertex::normal() const
    {
    ...
    __device__ __host__ bool scene<2>::silhouette_vertex::find_closest_silhouette_point(const float2 origin, const float max_radius_squared, float &distance, // TODO: use pointer
                                                                                        const bool flip_normal_orientation, const float min_radius_squared) const
    {
    ...
    const auto &scene<2>::get_bvh_device_ptr() const
    {
        if (p_bvh)
        {
            return bvh_dev;
    ...
    void scene<2>::compute_silhouettes()
    {
        silhouettes_h.clear();
    ...
    __device__ __host__
    scene<3>::triangle::triangle(const int3 vertex_indices, const int3 silhouette_indices,
                                 const thrust::device_vector<float3> &vertices_d, const thrust::device_vector<silhouette_edge> &silhouettes_d)
        : vertex_indices(vertex_indices), silhouette_indices(silhouette_indices), vertices(vertices_d.data().get()), silhouettes(silhouettes_d.data().get())
    {
    }
    ...
    __device__ __host__
        lbvh::aabb<float, 3>
        scene<3>::aabb_getter::operator()(const triangle &object) const noexcept
    {
    ...
    __device__ __host__
        lbvh::cone<float, 3>
        scene<3>::cone_getter::operator()(const triangle &object) const noexcept
    {
    ...
    __device__ __host__ float scene<3>::distance_calculator::operator()(const float4 point, const triangle &object) const noexcept
    {
    ...
    __device__ __host__ bool scene<3>::silhouette_distance_calculator::operator()(const float4 origin, const triangle &object, const float max_radius_squared, float &distance,
                                                                                  const bool flip_normal_orientation, const float min_radius_squared) const noexcept
    {
        // TODO: Implement
    ...
    template <typename VerticesInputIterator, typename IndicesInputIterator>
    scene<3>::scene(VerticesInputIterator vertices_first, VerticesInputIterator vertices_last, IndicesInputIterator indices_first, IndicesInputIterator indices_last)
        : vertices_h(vertices_first, vertices_last), vertices_d(vertices_h), indices_h(indices_first, indices_last)
    {
    }

    __device__ __host__ scene<3>::silhouette_edge::silhouette_edge(const int4 indices, const thrust::device_vector<float3> &vertices_d)
        : indices(indices), vertices(vertices_d.data().get())
    {
    }

    __device__ __host__ scene<3>::silhouette_edge::silhouette_edge(const thrust::device_vector<float3> &vertices_d)
        : indices(make_int4(-1, -1, -1, -1)), vertices(vertices_d.data().get())
    {
    }

    __device__ __host__ scene<3>::silhouette_edge::silhouette_edge()
        : indices(make_int4(-1, -1, -1, -1)), vertices(nullptr)
    {
    }

    __device__ __host__ aabb<float, 3> scene<3>::silhouette_edge::bounding_box() const
    {
    ...
    __device__ __host__ float3 scene<3>::silhouette_edge::centroid() const
    {
    ...
    __device__ __host__ bool scene<3>::silhouette_edge::has_face(int f_index) const
    {
        return f_index == 0 ? get(indices, 3) != -1 : get(indices, 0) != -1;
    }

    __device__ __host__ float3 scene<3>::silhouette_edge::normal(int f_index, bool do_normalize) const
    {
        int i, j, k;
        if (f_index == 0)
        {
    ...
    __device__ __host__ float3 scene<3>::silhouette_edge::normal() const
    {
    ...
    __device__ __host__ bool scene<3>::silhouette_edge::find_closest_silhouette_point(const float3 origin, const float max_radius_squared, float &distance,
                                                                                      const bool flip_normal_orientation, const float min_radius_squared) const
    {
        if (min_radius_squared >= max_radius_squared)
        {
    ...
    const auto &scene<3>::get_bvh_device_ptr() const
    {
        if (p_bvh)
        {
            return bvh_dev;
    ...
    int scene<3>::assign_edge_indices()
    {
        int E = 0;
    ...
    void scene<3>::compute_silhouettes()
    {
    ...
    inline __device__ __host__ thrust::pair<bool, float> scene<3>::intersect_test::operator()(const Line<float, 3> &line, const triangle &object) const noexcept
    {
    ...
    inline __device__ __host__ thrust::pair<bool, float> scene<2>::intersect_test::operator()(const Line<float, 2> &line, const line_segment &object) const noexcept
    {
