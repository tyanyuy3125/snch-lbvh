...
template <typename T, unsigned int dim> struct cone {
...
__device__ __host__ inline bool is_valid(const cone<T, dim> &c) noexcept {
    return c.half_angle >= T(0);
}

__device__ __host__ inline void compute_orthonormal_basis(const double3 &n, double3 *b1, double3 *b2) noexcept {
...
__device__ __host__ inline void compute_orthonormal_basis(const float3 &n, float3 *b1, float3 *b2) noexcept {
...
__device__ __host__ inline float project_to_plane(const float2 &n, const float2 &e) noexcept {
    // float2 b{-n.y, n.x};
...
__device__ __host__ inline double project_to_plane(const double2 &n, const double2 &e) noexcept {
    double2 b = make_double2(-n.y, n.x);
    double r = dot(e, cwiseabs(b));
    return std::abs(r);
}

__device__ __host__ inline float project_to_plane(const float3 &n, const float3 &e) noexcept {
    float3 b1, b2;
...
__device__ __host__ inline double project_to_plane(const double3 &n, const double3 &e) noexcept {
    double3 b1, b2;
    compute_orthonormal_basis(n, &b1, &b2);
...
__device__ __host__ inline bool overlap(const cone<float, 2> &bc, const float2 &o, const aabb<float, 2> &b, const float dist_to_box, float *min_angle_range, float *max_angle_range) noexcept {
...
__device__ __host__ inline bool overlap(const cone<double, 2> &bc, const double2 &o, const aabb<double, 2> &b, const double dist_to_box, double *min_angle_range, double *max_angle_range) noexcept {
    *min_angle_range = 0.0;
...
__device__ __host__ inline bool overlap(const cone<float, 3> &bc, const float4 &o, const aabb<float, 3> &b, const float dist_to_box, float *min_angle_range, float *max_angle_range) noexcept {
    *min_angle_range = 0.0f;
    *max_angle_range = static_cast<float>(M_PI_2);
...
__device__ __host__ inline bool overlap(const cone<double, 3> &bc, const double4 &o, const aabb<double, 3> &b, const double dist_to_box, double *min_angle_range, double *max_angle_range) noexcept {
    *min_angle_range = 0.0;
    *max_angle_range = M_PI_2;
...
__device__ __host__ inline float2 rotate(const float2 &u, const float2 &v, float theta) {
...
__device__ __host__ inline double2 rotate(const double2 &u, const double2 &v, double theta) {
...
__device__ __host__ inline float3 rotate(const float3 &u, const float3 &v, float theta) {
    float cos_theta = std::cos(theta);
    float sin_theta = std::sin(theta);
...
__device__ __host__ inline double3 rotate(const double3 &u, const double3 &v, double theta) {
    double cos_theta = std::cos(theta);
    double sin_theta = std::sin(theta);
...
__device__ __host__ inline cone<float, 2> merge(const cone<float, 2> &cone_a, const cone<float, 2> &cone_b, const float2 &origin_a, const float2 &origin_b, const float2 &new_origin) noexcept {
    cone<float, 2> ret;
...
__device__ __host__ inline cone<double, 2> merge(const cone<double, 2> &cone_a, const cone<double, 2> &cone_b, const double2 &origin_a, const double2 &origin_b, const double2 &new_origin) noexcept {
    cone<double, 2> ret;
    if(is_valid(cone_a) && is_valid(cone_b)) {
...
__device__ __host__ inline cone<float, 3> merge(const cone<float, 3> &cone_a, const cone<float, 3> &cone_b, const float4 &origin_a, const float4 &origin_b, const float4 &new_origin) noexcept {
    cone<float, 3> ret;
    if(is_valid(cone_a) && is_valid(cone_b)) {
...
__device__ __host__ inline cone<double, 3> merge(const cone<double, 3> &cone_a, const cone<double, 3> &cone_b, const double4 &origin_a, const double4 &origin_b, const double4 &new_origin) noexcept {
    cone<double, 3> ret;
    if(is_valid(cone_a) && is_valid(cone_b)) {
        auto axis_a = cone_a.axis;
        auto axis_b = cone_b.axis;
